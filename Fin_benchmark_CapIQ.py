            ...
            if not all_metrics_to_rank:
                ...
                if not api_key_for_call:
                    ...
                    if err:
                        ...
                        # Persist the analysis so it survives any rerun (incl. download clicks)
                        st.session_state["ai_audit_suggestions"] = text.strip()
                        # Clear any stale PDF (so it gets rebuilt for the new text once below)
                        st.session_state.pop("ai_audit_suggestions_pdf_bytes", None)

        # Always render whatever we have in session (this runs on every rerun)
        if st.session_state.get("ai_audit_suggestions"):
            st.markdown(st.session_state["ai_audit_suggestions"])

            # Build the PDF ONCE and cache it (so the download click doesn't recompute)
            ...
            # Download uses cached bytes; the rerun just re-renders the same content above
            st.download_button(
            ...
            st.info("Generate suggested audit areas to view them here.")
